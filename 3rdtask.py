...
import requests
import json
from env import AIDEV3_API,OPENAI_API_KEY

task = "JSON" 
data = f"https://centrala.ag3nts.org/data/{AIDEV3_API}/json.txt"
...
def verify_and_correct_answers(data):
    ...
    for item in data.get('test-data',[]):
        ...
        if expected_answer == correct_answer:
            ...
            incorrect_count += 1
            print(f"incorrect answer found '{expected_answer}' for question: {question}. Expected: {correct_answer} ")
...
print(response.text)
#    response = requests.post(data, data=json.dumps(payload), headers=headers)   
#    print(response.text)
